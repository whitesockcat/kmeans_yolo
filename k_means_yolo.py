...
def do_kmeans(n_anchors, boxes, centroids):
    ...
    for i in range(n_anchors):
        new_centroids[i].w /= len(groups[i]) + 1 # my edit + 1
        new_centroids[i].h /= len(groups[i]) + 1

    return new_centroids, groups, loss


def boxes_guide_anchor(boxes, n_anchors, loss_convergence, grid_size, iterations_num, plus):
    if plus:
...
def compute_centroids(label_path,n_anchors,loss_convergence,grid_size,iterations_num,plus):
    ...
    boxes_guide_anchor(boxes, n_anchors, loss_convergence, grid_size, iterations_num, plus)
